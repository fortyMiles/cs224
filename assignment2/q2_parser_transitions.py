class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            new_process = self.buffer[0]
            self.stack.append(new_process)
            del self.buffer[0]
        else:
            if transition == 'LA':
                head_index, depend_index = -1, -2
            elif transition == 'RA':
                head_index, depend_index = -2, -1
            else:
                raise NameError

            head = self.stack[head_index]
            dependent = self.stack[depend_index]
            self.dependencies.append((head, dependent))
            del self.stack[depend_index]
